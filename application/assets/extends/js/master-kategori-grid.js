var datatable,
tableTarget = '#table1',
ajaxUrl = baseUrl+'/master/kategori/',
ajaxSource = ajaxUrl+'json_grid',
laddaButton, formAction='add', idKategori
;
...
function load_table() {
    datatable = $(tableTarget).mDatatable({
        ...
        },
        {
            ...
            template: function (row, index, datatable) {
                return '\
                <button type="button" class="btn btn-outline-primary m-btn m-btn--icon m-btn--icon-only m-btn--outline-2x m-btn--pill m-btn--air" data-toggle="tooltip" title="Edit" onclick="edit('+row.id_kategori+')">\
...
function tambah() {
    ...
    $('#modal_form .modal-title').html('Tambah Kategori');
...
function edit(id_kategori='') {
    ...
    $('#modal_form .modal-title').html('Edit Kategori');
    $('#modal_form').modal('show');

    $.ajax({
        type:"GET",
        url: ajaxUrl+"json_get/"+id_kategori,
        beforeSend: function() {
            preventLeaving();
        ...
        success:function(response){
            ...
            if(obj.status == "OK"){
                $('#nama_kategori').val(obj.data.kategori['kategori']);
                if(obj.data.kategori.aktif == 1){
